...
\begin{lstlisting}[float=htbp,caption={Using Alice's pseudo-random number generator},label={generatedNumbers}]
...
\begin{lstlisting}[float=htbp,caption={The implementation of Alice's pseudo-random number generator},label={aliceRandomGenerator}]
...
\begin{lstlisting}[float=htbp,caption={The original implementation of Bob's parser},label={bobParser}]
...
\begin{lstlisting}[float=htbp,caption={The implementation of Bob's logging parser},label={bobLoggingParser}]
import dsl.Dsl.!!
...
\begin{lstlisting}[float=htbp,caption={The implementation of Bob's logging parser, the underscore placeholder version},label={bobLoggingParserUnderscore}]
...
\begin{lstlisting}[float=htbp,caption={The implementation of Bob's logging parser, the reset version},label={bobLoggingParserReset}]
...
\begin{lstlisting}[float=htbp,caption={Using Bob's parser},label={usingBobLoggingParserDelay}]
...
\begin{lstlisting}[float=htbp,caption={Carol's line splitter},label={carolLineSplitter}]
import dsl.Dsl.!!
...
\begin{lstlisting}[float=htbp,caption={Using Carol's line splitter},label={usingCarolLineSplitter}]
...
\begin{lstlisting}[float=htbp,caption={Dave's HTTP client},label={daveHttpClient}]
import dsl.task._
...
\begin{lstlisting}[float=htbp,caption={Using Dave's http client},label={usingDaveHttpClient}]
...
\begin{lstlisting}[float=htbp,caption={Using Dave's http client in parallel},label={usingDaveHttpClientInParallel}]
...
\begin{lstlisting}[float=htbp,caption={Erin's line of code counter, the monadic version},label={erinMonadicCounter}]
...
\begin{lstlisting}[float=htbp,caption={Erin's line of code counter, mixed \lstinline{Monad}-based and \lstinline{CanBuildFrom}-based LDKs},label={erinMixedCounter}]
...
\begin{lstlisting}[float=htbp,caption={Erin's line of code counter, using an \lstinline{OptionT} monad transformer},label={erinTransformerCounter}]
import scalaz._
import java.io.File
import dsl.keywords.Monadic, Monadic.implicitMonadic
import dsl.domains.scalaz._
...
\begin{lstlisting}[float=htbp,caption={The \lstinline{Yield} LDK, the nonadaptive version},label={NonadaptiveYield}]
...
\begin{lstlisting}[float=htbp,caption={The translated code for Alice's pseudo-random number generator},label={translatedAliceRandomGenerator}]
def aliceRandomGenerator(seed: Int): Stream[Int] = {
  val tmp1 = seed ^ (seed << 13)
  val tmp2 = tmp1 ^ (tmp1 >>> 17)
  val tmp3 = tmp2 ^ (tmp2 << 5)
...
\begin{lstlisting}[float=htbp,caption={The translated code for Bob's parser},label={translatedBobLoggingParser}]
import dsl.Dsl.!!
def bobLoggingParser(jsonContent: String, defaultValue: JSONType): Stream[String] !! JSONType = { (callback: JSONType => Stream[String]) =>
...
\begin{lstlisting}[float=htbp,caption={The ad-hoc polymorphism in \lstinline{Keyword}},label={Keyword}]
...
\begin{lstlisting}[float=htbp,caption={The type class to interpret \lstinline{cpsApply}},label={Dsl}]
...
\begin{lstlisting}[float=htbp,caption={The \lstinline{Yield} LDK, the adaptive version},label={Yield}]
...
\begin{lstlisting}[float=htbp,caption={The \lstinline{Yield} LDK, the adaptive version},label={continuationDsl}]
...
But what interesting is that an LDK can be considered as a more polymorphic version of monad as well. The interpreter for an LDK is a triple parametric \lstinline{Dsl} type class (Listing \ref{Dsl}), which contains only one \lstinline{interpret} function, whose type signature is \lstinline{(K, (A => D)) => D}, which is exactly as same as monadic bind operation when \lstinline{K} is \lstinline{F[A]} and \lstinline{D} is \lstinline{F[B]}. Thus, the interpreter for \lstinline{Monadic} LDK (See section \ref{Monadic programming}) can be implemented as a trivial forwarder to bind operation, as shown in Listing \ref{monadDsl}. In contrast, the reverse adapter is quite difficult, if not impossible, to be implemented.
