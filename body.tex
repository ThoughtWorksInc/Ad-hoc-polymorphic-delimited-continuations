...
\begin{figure}[h t b p]
  ...
  digraph {
  	...
  	subgraph cluster_code_block {
      graph [ label="\textrm{a code block that returns a \lstinline{Continuation}}" ]
      Continuation_0
      Continuation_1
      Continuation_2
      Continuation_dots [ label="{\vdots}" shape=none ]
      Continuation_n
    }
  }
  \end{dot2tex}
  ...
  \label{Continuation-architecture}
\end{figure}
...
A \lstinline{Continuation} itself contains the implementation of an operation, as shown in Figure~\ref{Continuation-architecture}. As a result, a delimited continuation can only be used in a function that returns the specified \lstinline{Domain}. Unlike ordinary \lstinline{Continuation}, in our approach, each operation is an ad-hoc polymorphic \lstinline{Keyword} to the \lstinline{Domain}, thus the it is can be interpreted differently according to the enclosing \lstinline{Domain}, as shown in Figure~\ref{dsl-architecture}.
...
When performing \lstinline{!}-notation on a \lstinline{Keyword} to produce a \lstinline{Value} inside a function whose return type is \lstinline{Domain}, the type class instance of \lstinline{Dsl[Keyword, Domain, Value]} is required. For example, Adaptive version of LDKs in Listing~\ref{NonadaptiveYield},  Listing~\ref{NonadaptiveAwait},  Listing~\ref{Yield-StreamFuture} and Listing~\ref{Await-StreamFuture} requires \lstinline{Dsl} instances implemented in Listing~\ref{Yield-generator-instance}, Listing~\ref{Yield-async-generator-instance}, Listing~\ref{Await-future-instance} and Listing~\ref{Await-async-generator-instance}.
...
In Section~\ref{Adaptive library-defined keywords}, we have present how to create a LDK for different domains, interpreted by different implementations of \lstinline{Dsl} type class instances. In this section, we will discuss derived \lstinline{Dsl} type class instances for an LDK, available in derived domains.

A derived domain means a domain whose type signature contains another domain, and a derived \lstinline{Dsl} means a \lstinline{Dsl} whose implementation internally invokes another \lstinline{Dsl}. The need of derived domains and \lstinline{Dsl}s usually comes when introducing a new LDK to an unsupported domain \footnote{Monad transformers and effect handlers are alternative approaches to \lstinline{Dsl} derivation, which will be discussed in Section~\ref{Monad transformers} and Section~\ref{Effect handlers}.}.

For example, in addition to \lstinline{yield}, Python generators also allow the \lstinline{return} and \lstinline{yield from} keywords. A generator that contains both \lstinline{yield} and \lstinline{return} keywords can be invoked by \lstinline{yield from} in another generator. The elements \lstinline{yield}ed by the former generator will be added into the latter generator, and the return value of the former generator can be in the latter generator, too. An example of \lstinline{return} and \lstinline{yield from} is shown in Listing~\ref{returnable_generator}.
...
def returnable_generator(early_return: bool) -> Generator[str, None, int]:
...
def final_generator() -> Iterator[str]:
  yield 'before returnable_generator'
  v = yield from returnable_generator(True)
  yield 'after returnable_generator'
  yield f'the return value of returnable_generator is {v}'

# Output: ['before returnable_generator', 'inside returnable_generator', 'early return', 'after returnable_generator', 'the return value of returnable_generator is 1']
print(list(final_generator()))
\end{lstlisting}

Unlike generators in Section~\ref{Implementing LDKs as ordinary delimited continuations}, \lstinline{returnable_generator} has an additional ability of returning values, thus its return type becomes to \lstinline{Generator[str, None, int]}, where \lstinline{str} is the iterator element type and \lstinline{int} is the type to return \footnote{Note that the declared return type and the type to return are different in Python generators}.

When porting \lstinline{return} and \lstinline{yield from} to Scala, the return type should indicate both the element type and the type to return, thus \lstinline{Stream} is not applicable for return type any more. We can instead use the return type \lstinline{Continuation[Stream[String], Int]}, as shown in Listing~\ref{returnableGenerator}. It accepts a callback function \lstinline{Int => String}, which can handle the \lstinline{Int} value being \lstinline{Return}ed.

\begin{lstlisting}[caption={Use \lstinline{Shift} and \lstinline{Return} in LDK-based generators},label={returnableGenerator}]
def returnableGenerator(earlyReturn: Boolean): Continuation[Stream[String], Int] = {
...
def finalGenerator(): Stream[String] = {
  !Yield("before returnableGenerator")
  val v = !Shift(returnableGenerator(true))
  !Yield("after returnableGenerator")
  !Yield(s"the return value of returnableGenerator is $v")
  Stream.empty
}

// Output: List(before returnableGenerator, inside returnableGenerator, early return, after returnableGenerator, the return value of returnableGenerator is 1)
println(finalGenerator().toList)
\end{lstlisting}

We also create new LDKs \lstinline{Shift} and \lstinline{Return} as the replacements of Python's \lstinline{yield from} and \lstinline{return}, respectively, as shown in Listing~\ref{Shift} and Listing~\ref{Return}. We don't use native Scala \lstinline{return} because it lacks the ability of ``lifting'' values like Python \lstinline{return} does in generators.
...
As we had split LDK declaration (i.e. subtypes of \lstinline{Keyword}) from its implementation (i.e. \lstinline{Dsl} type class instances), the following type class instances is required to compile Listing~\ref{returnableGenerator}:

\begin{enumerate}
  \item \lstinline{Dsl[Yield[String], Stream[String], Unit]}
  \label{DslYield}

  \item \lstinline{Dsl[Shift[Stream[String], Int], Stream[String], String]}
  \label{DslShift}
  
  \item \lstinline{Dsl[Return[Int], Continuation[Stream[String], Int], Nothing]}
  \label{DslReturn}
  
  \item \lstinline{Dsl[Yield[String], Continuation[Stream[String], Int], Unit]}
  \label{DslYieldContinuation}
\end{enumerate}

As we had introduced in simple generators, \lstinline{Dsl} instance \ref{DslYield}, required by \lstinline{finalGenerator}, can be resolved from the type class instance defined in Listing~\ref{Yield-generator-instance}. Also in \lstinline{finalGenerator}, \lstinline{Dsl} instance \ref{DslShift} should forward \lstinline{cpsApply} call to the underlying \lstinline{continuation} of the \lstinline{Shift} LDK, as shown in Listing~\ref{shiftDsl}. Sine the \lstinline{!Return} LDK immediately return from the current function, the implementation of \lstinline{Dsl} instance \ref{DslReturn} should skip the rest part of the function, which is captured as a callback function as parameter of \lstinline{cpsApply}, as shown in Listing~\ref{returnDsl}.
...
implicit def returnDsl[A]: Dsl[Return[A], A, Nothing] =
...
\lstinline{Dsl[Return[Int], Continuation[Stream[String], Int], Nothing]}
...
\subsection{Effect handlers}\label{Effect handlers}
