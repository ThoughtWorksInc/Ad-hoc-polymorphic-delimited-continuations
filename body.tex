...
\subsection{Using multiple keywords at once}\label{Using multiple keywords at once}
...
The usage of \lstinline{Task} can be similar to previous examples in Section \ref{Using multiple keywords at once}, but we also provide \lstinline{blockingAwait} and some other utilities under the implicit class \lstinline{com.thoughtworks.dsl.task.TaskOps} to ease the usage (Listing \ref{usingDaveHttpClient}).
...
Another useful keyword for asynchronous programming is \lstinline{Fork}, which duplicate the current control flow, and the child control flows are executed in parallel, similar to the POSIX \lstinline{fork} system call.
...
Despite keywords directly implemented in \textit{Dsl.scala}, we also provide some adapters to monads and other type classes.

The built-in \lstinline{Monadic} keyword can be used as an adaptor to \lstinline{scalaz.Monad}, to create monadic code from imperative syntax, similar to the !-notation in Idris.

For example, suppose Erin is creating a program that counts lines of code under a directory. She uses the \lstinline{Monadic} keyword store the result in a \lstinline{Stream} of line count of each file (Listing \ref{erinMonadicCounter}).
...
There is a \lstinline{Each} keyword in \textit{Dsl.scala} to extract each element in a Scala collection, based on {CanBuildFrom} type class instead of monads. The \lstinline{Each} behavior is similar to \lstinline{Monadic}, except the collection type can vary.

Thus, Erin can extract each element from an \lstinline{Array} with the help of \lstinline{Each} keyword in Listing \ref{erinMixedCounter},
even when the enclosing domain is still a \lstinline{Stream}.

\begin{lstlisting}[caption={Erin's line of code counter, mixed \lstinline{Monad}-based and \lstinline{CanBuildFrom}-based keywords},label={erinMixedCounter}]
...
As shown the \lstinline{erinMixedCounter}, Dsl.scala allows non-monadic keywords like \lstinline{Each} works along with monads, unlike Haskell's do-notation or Idris's !-notation.
...
Erin wants to use \lstinline{OptionT} monad transformer to mark those failed file as a \lstinline{None} (Listing \ref{erinTransformerCounter}).
