...
\begin{lstlisting}[float=htbp,language={Haskell},caption={Running \lstinline{f} effectfuly in REPL},label={impure-instances}]
> f :: IO ()
Yield foo
Get
Return baz
...
In brief, the Haskell implementation \textit{control-dsl} has a more powerful ability of type inference, which the \lstinline{do}-notation is more verbose than !-notation that we used with \textit{Dsl.scala}.

\section{Future work}

TODO: ``Ad-hoc polymorphic'' ``polymorphic delimited continuation''
