...
The usage of \lstinline{Task} is similar to previous examples in Section \ref{Using multiple keywords at once}. But we also provides \lstinline{blockingAwait} and some other utilities under the implicit class \lstinline{com.thoughtworks.dsl.task.TaskOps}.
...
Thus, Erin can extract each element from an \lstinline{Array} with the help of \lstinline{Each} keyword,
even when the return type (or the domain) is still a \lstinline{Stream}.

\begin{lstlisting}[caption={Erin's line of code counter, mixed \lstinline{Monad}-based and \lstinline{CanBuildFrom}-based keywords},label={erinMonadicCounter}]
