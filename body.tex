...
finalGenerator.foreach(println)
\end{lstlisting}
...
\begin{enumerate}
  \item \lstinline{Dsl[Yield[String], Stream[String], Unit]} \\ (required by \lstinline{!Yield} LDKs in \lstinline{finalGenerator})
  \label{DslYield}

  \item \lstinline{Dsl[Shift[Stream[String], Int], Stream[String], String]} \\ (required by \lstinline{!Shift} LDKs in \lstinline{finalGenerator})
  \label{DslShift}
  
  \item \lstinline{Dsl[Return[Int], Continuation[Stream[String], Int], Nothing]} \\ (required by \lstinline{!Return} LDKs in \lstinline{returnableGenerator})
  \label{DslReturn}
  
  \item \lstinline{Dsl[Yield[String], Continuation[Stream[String], Int], Unit]} \\ (required by \lstinline{!Yield} LDKs in \lstinline{returnableGenerator})
  \label{DslYieldContinuation}
\end{enumerate}

As we had introduced in Section~\ref{Implementing LDKs as ordinary delimited continuations}, \lstinline{Dsl} instance \ref{DslYield} can be resolved as \lstinline{yieldDsl[Stirng, String]} from the type class instance defined in Listing~\ref{Yield-generator-instance}. \lstinline{Dsl} instance \ref{DslShift} should forward \lstinline{cpsApply} call to the underlying \lstinline{continuation} of the \lstinline{Shift} LDK, as shown in Listing~\ref{shiftDsl}. Since a \lstinline{!Return} LDK immediately return from the current function, the implementation of \lstinline{Dsl} instance \ref{DslReturn} should skip the rest part of the function, which is captured as a callback function passed to \lstinline{cpsApply}, as shown in Listing~\ref{returnDsl}.
...
\lstinline{Dsl} instance \ref{DslYieldContinuation} should allow registering a callback of \lstinline{Unit => Continuation[Stream[String], Int]} and then return a new \lstinline{Continuation[Stream[String], Int]}. Since the return type of the newly created \lstinline{Continuation} is \lstinline{Stream[String]}, the \lstinline{Yield[String]} keyword can be performed by existing \lstinline{Dsl} instance \ref{DslYield}, the extracted value \lstinline{v} is then passed to \lstinline{handler} to continue the execution of rest program, as shown in Listing~\ref{yieldContinuationDsl}.
...
Now the required \lstinline{Dsl} instance \ref{DslYieldContinuation} can be resolved from either \lstinline{yieldContinuationDsl}, or, more generically, \lstinline{continuationDsl(yieldDsl)}. The ability of \lstinline{Dsl} derivation enables heterogeneous LDKs in one function, whose return type is composed from the required domain of those LDKs. Section~\ref{Multiple mutable states} contains a more complex example TODO
...
\subsection{Multiple mutable states}\label{Multiple mutable states}
