...
% The implementation of Monix tasks, Cats effects, Scalaz Concurrent are monadic data types based on sealed ADTs.
...
As we did in sum benchmarks, we created some benchmarks to maximize the performance for Cartesian product. Our benchmarks create the Cartesian product from \lstinline{traverseM} for Scala Future, Cats Effect, Scalaz Concurrnet and Monix Tasks. Listing \ref{RawCartesianPruduct.scalaFuture} shows the benchmark code for Scala Future.

\begin{lstlisting}[caption={Cartesian product for Scala Future, based on \lstinline{traverseM}},label={RawCartesianPruduct.scalaFuture}]
...
Scala Async or \lstinline{for} comprehension is used in element-wise task \lstinline{cellTask}, and the collection manipulation \lstinline{listTask} is kept as manually written higher order function calls, because neither Scala Async nor \lstinline{for} comprehension supports \lstinline{traverseM}.
...
\subsubsection{The performance impact of multi-domains}
...
\end{table}
%also want to know the performance of loops 
...
% In our implementation, the native effect systems is continuation of
...
% Our compiler plugin generates very efficient code.
...
The idea of LDK is simple. It is merely an ad-hoc polymorphic delimited continuation, interpreted by a domain-specific type class. However, in split of LDK and its interpreter dramatically improves both extensibility and computational performance in comparison to ordinary delimited continuation.

% This paper describes a simple improvement on delimited continuation. We split
%  A  domain-specific polymorphic function. the LDK  xx
