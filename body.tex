...
Monad transformers are monads derived from other monads. ``Monad transformer'' is to ``monad'' as ``\lstinline{Dsl} derivation'' is to ``\lstinline{Dsl}''. The monadic data type can be composed at type level as a chain of monad transformers, so that various operations can be lifted to the same nested transformed monadic data type, which can be then used in a single monadic code block. TODO: ??????????????????monad transformer????????????

\begin{figure}[h t b p]
  \begin{dot2tex}[dot,autosize,graphstyle={scale=0.78,transform shape}]
  digraph {
    rankdir=TB

    M [ label="An atomic operation" ]
    T1 [ label="The transformed monadic data type"]
    T2 [ label="The transformed monadic data type"]
    M -> T1 [ label="transformed \lstinline{lift}" ]
    T1 -> T2 [ label="transformed \texttt{bind}" ]
  }
  \end{dot2tex}

  \caption{What's happened when executing an atomic operation in a monad transformer}
...
This is not a problem in our \lstinline{Dsl} derivation approach. 
