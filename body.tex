...
\begin{lstlisting}[caption={Using Dave's http client in parallel},label={usingHttpClientInParallel}]
...
In addition to \lstinline{Fork}, we also provide the \lstinline{Each} LDK, whose type signature is identical with \lstinline{Fork}, to sequentially execute tasks. If the \lstinline{!Fork} is replaced to \lstinline{!Each}, those URLs will be fetched sequentially. Other usage of \lstinline{Each} LDK will be introduced in Section~\ref{Scala's for-comprehension}.
