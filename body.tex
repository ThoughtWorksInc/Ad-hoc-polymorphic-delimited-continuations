...
\begin{figure}[h t b p]
  ...
  digraph {
    ...
    M -> T1 [ label="transformed lift", texlbl="transformed \texttt{lift}" ];
    T1 -> T2 [ label="transformed bind", texlbl="transformed \lstinline{bind}" ];
  }
  \end{dot2tex}
...
The inefficient lifting can be avoided in our \lstinline{Dsl} derivation approach, since an LDK already represents an operation for any compatible domains.
...
\begin{figure}[h t b p]
  ...
  digraph {
    ...
    M -> T [ label="transformed cpsApply", texlbl="transformed \texttt{cpsApply}" ];
  }
  \end{dot2tex}
