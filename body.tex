...
The difference is that a \lstinline{Cont} itself contains the implementation of an operation while in our approach each operation is ad-hoc polymorphic to the \lstinline{Domain} by separating to the syntactic \lstinline{Keyword} and its implementation \lstinline{Dsl}, as shown in Figure~\ref{cont-architecture} and Figure~\ref{dsl-architecture}.
...
Our goal is making the control flow of a programming language be extensible. In this section, we will present the approach used in \textit{Dsl.scala}, to custom programming language features as library-defined keyword (LDK) \footnote{ Code listings shown in Section~\ref{Basic concepts} are not exactly the same as the implementation in \textit{Dsl.scala}, instead, these implementations of LDKs are modified or simplified for the purpose of introducing the concept of the LDK approach more clearly.}. Multiple LDKs used in one function together make up a custom language for specific domain. No metaprogramming knowledge is required for either keyword authors or keyword users.

\subsection{Implementing LDKs as ordinary delimited continuations}
...
\subsection{CPS transformation}\label{CPS transformation}
...
\subsection{Collaborative library-defined keywords}
...
\begin{description}
  \item[The core library] contains the definition of the \lstinline{Dsl} type class and \lstinline{Keyword}, the super type of LDKs. They are slightly different from the definition in Listing~\ref{Dsl} and Listing~\ref{Keyword}.
  \begin{itemize}
    \item The \lstinline{apply} method in \lstinline{Dsl} and \lstinline{Keyword} is renamed to \lstinline{cpsApply}.
    \item There is an additional dummy method \lstinline{unary_!} defined in \lstinline{Keyword}, so that !-notation can be treated as a method call from the point view of type checker when our compiler plug-ins is not enabled. The \lstinline{unary_!} method is especially useful for IntelliJ IDEA\footnote{https://www.jetbrains.com/idea/}'s built-in type checker, preventing the edit window in the IDE from being red marked.
  ...
  \end{itemize}
  \item[Compiler plug-ins] performs CPS-transformation as described in Section~\ref{CPS transformation}. There are two compiler plug-ins in \textit{Dsl.scala}: \lstinline{ResetEverywhere} and \lstinline{BangNotation}. The \lstinline{ResetEverywhere} plug-in adds a \lstinline{@reset} annotation to the block of every method in source code, and the \lstinline{BangNotation} plug-in perform name-based CPS-transformation according to the control operator \lstinline{!} and \lstinline{@reset} annotation, which are equivalent \lstinline{shift} and \lstinline{reset} control operators to described in \cite{Danvy89afunctional}, respectively. In addition to block expressions mentioned in Listing~\ref{cps-block}, all other Scala first-class control flows are transformed to CPS form by the \lstinline{BangNotation} plug-in \footnote{Note that the \lstinline{for} expression is not converted as it is not a first-class control flow but method calls.}.
...
% \subsection{Difference between Dsl.scala and Section~\ref{Basic concepts}}

% \subsection{The implementation of CPS transformation compiler plug-ins}

% \subsection{Built-in LDKs}
