...
Ordinary delimited continuations can be defined as a function to register a callback function (Listing~\ref{Cont}), which is similar to the signature of \lstinline{Dsl} type class.
...
In our \textit{Dsl.scala}-based generator, the return type is replaced to \lstinline{Stream}, which can be considered as the immutable version of \lstinline{Iterator}, and the compiler-defined keyword \lstinline{yield} is replaced to library-defined keyword \lstinline{Yield}. Listing~\ref{xorShiftRandomGenerator} is an example to create an Xorshift \cite{marsaglia2003xorshift} pseudo-random number generator that returns an infinite stream of generated numbers. 
...
\begin{lstlisting}[caption={An Xorshift pseudo-random number generator in \textit{Dsl.scala}},label={xorShiftRandomGenerator}]
...
For simple use case such as \lstinline{xorShiftRandomGenerator}, LDKs can be implemented as ordinary delimited continuations. Listing~\ref{NonadaptiveYield} shows an implementation of the \lstinline{Yield} LDK, as a delimited continuation \footnote{The implementation of \lstinline{Yield}, \lstinline{Await} and \lstinline{Monadic} is present here for the purpose of introducing ordinary delimited continuation only. It's not the actual approach to implement \lstinline{Yield} in \textit{Dsl.scala}.}, in which the \lstinline{Yield} LDK creates infinite \lstinline{Stream}s by capturing \lstinline{handler} into a lazily evaluated \lstinline{Stream.Cons}.
...
\subsection{Adaptive library-defined keywords}
