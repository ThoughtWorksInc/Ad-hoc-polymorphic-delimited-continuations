...
For simple use case such as \lstinline{xorShiftRandomGenerator}, LDKs can be implemented as ordinary delimited continuations. Listing~\ref{NonadaptiveYield} shows an implementation of the \lstinline{Yield} LDK, as a delimited continuation \footnote{Listing~\ref{NonadaptiveYield} is present here for the purpose of introducing ordinary delimited continuation only. It's not the actual approach to implement \lstinline{Yield} in \textit{Dsl.scala}.}, in which the \lstinline{Yield} LDK creates infinite \lstinline{Stream}s by capturing \lstinline{handler} into a lazily evaluated \lstinline{Stream.Cons}.
