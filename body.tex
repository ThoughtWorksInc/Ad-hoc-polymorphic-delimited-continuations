...
Note that the created collection is a \lstinline{Set}, which is different from the type of source collection. In addition, our LDK-base collection comprehension allows passing heterogeneous collection types to \lstinline{!Each}. You can even create code like \lstinline{def f = Seq(!Each(Array('L', 'D', 'K')) * !Each(Set(1, 2)))}.
...
Since \lstinline{Each} works in any function that returns a collection, it can be used in \lstinline{Stream} functions. 

TODO

\section{Benchmarks}\label{Benchmarks}

We created some benchmarks to evaluate the computational performance of code generated by our compiler plug-in for LDKs, especially, we are interesting how !-notation and other direct style DSL affect the performance in an effect system that support both asynchronous and synchronous effects.
...
\begin{table}[htbp]
  \begin{tabular}{l|l}
    ...
    \hline
    The \texttt{Task} LDK & !-notation provided by \textit{Dsl.scala} \\
