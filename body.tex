...
The use case of Bob's parser demonstrates how to enable additional LDKs into existing functions. There are only few changes between \lstinline{bobLoggingParserDelay} and the original \lstinline{bobParser} in order to enable the \lstinline{Yield} LDK. Generally, an LDK user can introduce a new domain (i.e. the return value) to an existing method with the two changes:
...
All the LDKs used in Section \ref{Using library-defined keywords} are optional libraries, which depend on a common compiler-time transformation rule, which is implemented as a Scala compiler plug-in.
